...
{
  // create bigmat and store first block
  Eigen::MatrixXcd thisbigmat( (jell+1)*drc2, N2 );
  thisbigmat.setZero();
  if (!filtered)
  {
    std::cout << "Filtered indices and propagators must be computed first!\n";
  ...
  thisbigmat.block(0, 0, drc2, N2) = BmatR;

  // this will hold our propagator chain
  Eigen::MatrixXcd thisAmat;
  thisAmat.setIdentity( N, N );
    
  for (int j=1; j<=jell; ++j)
  {
    thisAmat = thisAmat * fprops[J - j];
    MatrixXcdRowMajor Result(drc2, N2);
    ...
    for (int k=0; k<drc2; ++k)
    {
      ...
      // Evaluate in two steps to guarantee two clean BLAS GEMM calls per batch
      Eigen::MatrixXcd temp = thisAmat.conjugate() * BmatRk;
      Rk.noalias() = temp * thisAmat.transpose();
    }
    mkl_set_num_threads(56);
    thisbigmat.block(j*drc2, 0, drc2, N2) = Result;
  }
  return thisbigmat;
}
...
Eigen::MatrixXcd memoryModel::qprop(int jell)
{
  ...
  for (int k=jell; k<nsteps; ++k)
  {
    ...
    Eigen::MatrixXcd thisbigmat = bigmatBuildLocal(k, jell);
    // Eigen::CompleteOrthogonalDecomposition<Eigen::MatrixXcd> cod(thisbigmat);
    // cod.setThreshold(1e-10);
    // Eigen::VectorXcd PreconVec = cod.solve(qhist.reshaped());
    Eigen::VectorXcd PreconVec = pseudoInverseVec(qhist.reshaped(), thisbigmat, tol);
    ...
    for (int i=0; i<numdelays; ++i)
    {
      ...
      else if ((J-jell) >= offstep)
      {
        if ((J-jell) == offstep)
        {
          ...
          rdmprops[i] = BmatR * pseudoInverse(this->bigmat, this->tol);
        }
        Eigen::MatrixXcd qhist = pred1rdms[i].block(0, J-jell, drc2, jell+1).rowwise().reverse();
