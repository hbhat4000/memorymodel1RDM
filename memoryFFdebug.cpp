...
int main(void)
{
  ...
  for (int k=0; k<drc2; ++k)
    for (int l=0; l<drcCI2; ++l)
      std::cout << "BmatT(" << k << "," << l << ") = " << BmatT(k,l) << "\n";
  */
  // using RowMajorMatrix = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>;
  // Eigen::Map<const RowMajorMatrix> BmatT(Bten.data(), drcCI*drcCI, drc*drc);
  ...
  int nsteps = 10;
  std::cout << "About to propagate for " << nsteps << " steps\n";
  ...
  /*
  for (int m=0; m<thisqprop.rows(); ++m)
    for (int n=0; n<thisqprop.cols(); ++n)
      std::cout << "thisqprop(" << (m+1) << "," << (n+1) << ") = " << thisqprop(m,n) << "\n";
  */

  for (int k=delay; k<nsteps; ++k)
  {
    Eigen::MatrixXcd temp = pred1rdms.block(0, k-delay, drc2, delay+1);
    Eigen::MatrixXcd temp2 = (temp.colwise() - bvec).reshaped();
    Eigen::MatrixXcd temp3 = thisqprop * temp2;
    pred1rdms.col(k+1) = bvec + temp3;
    /*
    for (int m=0; m<temp2.rows(); ++m)
      for (int n=0; n<temp2.cols(); ++n)
        std::cout << "temp2(" << (m+1) << "," << (n+1) << ") = " << temp2(m,n) << "\n";
    for (int m=0; m<temp3.rows(); ++m)
      for (int n=0; n<temp3.cols(); ++n)
        std::cout << "temp3(" << (m+1) << "," << (n+1) << ") = " << temp3(m,n) << "\n";
    for (int m=0; m<pred1rdms.col(k+1).rows(); ++m)
      for (int n=0; n<pred1rdms.col(k+1).cols(); ++n)
        std::cout << "pred1rdms.col(k+1)(" << (m+1) << "," << (n+1) << ") = " << pred1rdms.col(k+1)(m,n) << "\n";
    */
  } 
