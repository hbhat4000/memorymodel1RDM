...
int main(int argc, char** argv)
{
  ...
  options.add_options()
  ("h,help", "Print usage")
  ("smax", "Max number of states to put in initial superposition", cxxopts::value<int>())
  ...
  if (result.count("smax")==0)
  {
    std::cout << "Must specify smax!\n";
    return 1;
  }
  else
    smax = result["smax"].as<int>();
  ...
  // this matrix corresponds to the good columns in goodCols!
  Eigen::MatrixXi binmat = Eigen::MatrixXi::Zero(drcCI, drcCI);
  for (int ij=0; ij<goodCols.size(); ++ij)
  {
    int trow = goodCols[ij] / drcCI;
    int tcol = goodCols[ij] % drcCI;
    binmat(trow, tcol) = 1;
  }
  ...
  int sfs = spin_families[0].size();
  if (smax > sfs)
  {
    smax = sfs;
    std::cout << "Adjusting smax down to " << sfs << "\n";
  }
  for (int j=0; j<smax; j++)
  {
    int goodstate = spin_families[0][j];
  ...
  */
  ...
  if (savemae)
  {
    std::filesystem::path p(infile);
    std::string stem = p.stem().string();
    std::string filename = stem + "_" + std::to_string(dt) + "_" + std::to_string(delay) + "_" + std::to_string(smax) + ".txt";
    std::filesystem::path dir(outpath);
    std::filesystem::path outfile = dir / filename;
    std::ofstream out(outfile);
    out << maestr << "\n";
